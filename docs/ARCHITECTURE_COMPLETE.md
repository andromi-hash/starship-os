...
> **Author**: Opencode / Starship Engineering
...
| **Wildcard subjects** | Subject pattern `starship.agent.*.command.>` lets any agent subscribe to a single wildcard, making it trivial to add new agents without touching existing infrastructure. |
...
### 1.2 Why a File-Based Status Bridge (`/tmp/starship-status.json`)?

```
NATS ──→ starship-status.py ──→ /tmp/starship-status.json ──→ All 5 UI layers
```
...
| **Debuggable** | You can `cat /tmp/starship-status.json` at any time or pipe it to `jq` for debugging. |
...
| `starship_tray.py` | GTK3 StatusIcon tray indicator — colored dot, agent menu, quick-launch |
...
| `scheduler.py` | Cron scheduler. Reads schedules from `config.yaml`, triggers `starship.workflow.<name>`. |
...
| `cmd/root.go` | Root command `starship`. |
| `cmd/agent.go` | `starship agent {run,status,stop}` |
| `cmd/ping.go` | `starship ping` — NATS connectivity test. |
| `cmd/skill.go` | `starship skill {list,show,trigger}` |
| `cmd/system.go` | `starship system health` |
| `cmd/telemetry.go` | `starship telemetry` — reads latest from NATS. |
| `cmd/version.go` | `starship version` |
| `cmd/workflow.go` | `starship workflow {run,list}` |
...
| `start-tray.sh` | Launch `starship_tray.py` with PID tracking. |
...
| `message_history.py` | JetStream consumer. Stores all `starship.agent.>` messages to `/tmp/starship-history/`. |
...
| File | Purpose |
|---|---|
| `starship-nats.service` | Starts `nats-server` with production config. |
| `starship-staragent.service` | Starts StarAgent binary. BindsTo `nats`. |
| `starship-agents.service` | Starts `agent_daemon.py`. BindsTo `nats`. |
| `starship-dashboard.service` | Starts `dashboard/server.py`. After agents. |
...
| `starship-agent-mesh.target` | Target requiring NATS + StarAgent + agent template units. |
| `starship-agent@.service` | Template unit (`%i` = proxy/romi/ergo). |
| `starship-dashboard.target` | Target for dashboard stack. |
| `starship-dashboard-web.service` | Dashboard HTTP server. |
| `starship-status-bridge.service` | NATS→JSON bridge. |
...
| File | Purpose |
|---|---|
| `starship-indicator.py` | GTK3 AppIndicator — reads `status.json`, colored icons, menu with commands. |
| `starship-status.py` | NATS→JSON bridge: subscribes status/telemetry/events, writes to `/tmp/starship-status.json`. |
...
| `metadata.json` | Desklet UUID `starship-os@starship-os`. |
...
| File | Purpose |
|---|---|
| `starship.conkyrc` | Conky config — top-right overlay, agent status + telemetry via `execi python3 -c`. |
...
| `src/main.rs` | Every 10s collects CPU, memory, disk, network → publishes `starship.telemetry`. |
...
│                                   ▼                                      │
│                  ┌──────────────────────────────────┐                    │
│                  │ /tmp/starship-status.json         │                    │
│                  │ /tmp/starship-history/            │                    │
│                  └──────────┬───────────────────────┘                    │
├─────────────────────────────┼───────────────────────────────────────────┤
│                       BUS LAYER                                          │
│                  ┌──────────────────────┐                                │
...
|---|---|---|---|
| **Telemetry** | StarAgent → NATS `starship.telemetry` → status bridge → `status.json` → all UIs | 2–10s | JetStream TELEMETRY |
| **Agent Command** | User (UI/CLI) → NATS `starship.agent.<name>.command.<cmd>` → Agent Daemon → Ollama → response → NATS reply → UI | 5–120s | JetStream AGENTS |
| **Agent Status** | Agent Daemon → NATS `starship.agent.<name>.status` → status bridge → `status.json` → all UIs | 1–5s | JetStream AGENTS |
| **Workflow** | Scheduler/User → NATS `starship.workflow.<name>` → Workflow Engine → multi-agent commands → collection → response | 30–300s | JetStream AGENTS |
| **Message History** | All NATS `starship.agent.>` → JetStream AGENTS stream → Message History Consumer → `/tmp/starship-history/` | <1s | File + JetStream |
...
dashboard/server.py (Web)
  ...
  ├── NATS: 127.0.0.1:4222
  ├── File: /tmp/starship-status.json, /tmp/starship-history/
...
agent_daemon.py (×3: proxy, romi, ergo)
  ...
  ├── File: agents/*.yaml, skills/*/SKILL.md, /tmp/starship-status.json
...
scheduler.py
  ...
  └── NATS: publishes starship.workflow.<name>

workflows.py
  ├── Python: json, asyncio, logging, subprocess, datetime
  └── NATS: publishes starship.agent.*.command.* / subscribes replies

message_history.py
  ├── Python: nats-py, json, asyncio, logging, pathlib, datetime
  ├── NATS: subscribes starship.agent.> (JetStream, durable)
  └── File: /tmp/starship-history/<date>.jsonl

cinnamon/desklet.js
  ├── Cinnamon APIs: St, Desklet, Mainloop, Gio, Settings
  └── File: /tmp/starship-status.json

conky/starship.conkyrc
  ├── Binary: conky
  └── File: /tmp/starship-status.json (via python3 -c)

cli/starship (Go binary)
...
```
/tmp/starship-status.json
  ├── Written by: tray/starship-status.py (NATS subscriber)
  ├── Read by: bridge_console.py, starship_tray.py, dashboard/server.py, desklet.js, conkyrc
  └── Schema: { agents: { <name>: { status, model, last_seen } }, telemetry: { cpu, memory, disk, net }, messages: [...] }
```

This means **adding a new UI layer** is trivial: write anything that reads `/tmp/starship-status.json` and renders it.
...
| Package | Used By |
|---|---|
| `nats-py` | server.py, agent_daemon.py, starship-status.py, message_history.py, query_agents.py |
...
| `PyGObject` (gi) | bridge_console.py, starship_tray.py, starship-indicator.py |

### 4.5 Build-Time Dependencies

| Artifact | Build Command | Input | Output |
|---|---|---|---|
| `cli/starship` | `cd cli && go build -o starship` | `main.go`, `cmd/*.go` | Go binary |
...
|---|---|---|---|---|
| `starship.telemetry` | StarAgent (Rust) | status-bridge, CLI telemetry | `{ cpu, memory_used, memory_total, disk_used, disk_total, rx_bytes, tx_bytes, timestamp }` | Every 10s |
| `starship.agent.<name>.command.>` | — | agent_daemon (wildcard) | — | N/A (subscription) |
| `starship.agent.<name>.command.<cmd>` | dashboard, workflows, CLI, scripts | agent_daemon | `{ command, args?, reply_to?, timestamp }` | On user/trigger |
| `starship.agent.<name>.status` | agent_daemon, CLI ping | status-bridge, message-history | `{ agent, status, command?, response?, timestamp }` | On status change |
| `starship.agent.<name>.event.*` | agent_daemon | status-bridge | `{ event, agent, data }` | On agent events |
| `starship.workflow.<name>` | scheduler, dashboard, CLI | workflows engine | `{ workflow, payload, reply_to? }` | On cron/user |
| `starship.workflow.<name>.<step>` | workflows engine | — | varies | Workflow steps |
| `starship.skill.<name>` | (future) | message-history | `{ skill, payload }` | Future use |
| `starship.history.latest` | message_history | dashboard (future) | `{ subject, data, timestamp }` | Per stored msg |
| `starship.reply.<ts>` | dashboard, scripts | dashboard, scripts | `{ agent, response, status, timestamp }` | Per command |

### 5.2 Subject Pattern Grammar

```
starship.agent.<agent>.command.<cmd>       # Publish: dashboard, workflows, CLI
                                            # Subscribe: agent_daemon (wildcard)
                                            
starship.agent.<agent>.status              # Publish: agent_daemon (after processing)
                                            # Subscribe: status bridge, message history

starship.agent.<agent>.event.<event>       # Publish: agent_daemon
                                            # Subscribe: status bridge

starship.workflow.<name>                   # Publish: scheduler, dashboard, CLI
                                            # Subscribe: workflows engine

starship.skill.<name>                      # Publish: future trigger system
...
User/CLI/Dashboard
    ...
    │  (1) Publish to:
    ▼
starship.agent.<name>.command.<command>
    │
...
Parse command → Build Ollama prompt → POST http://127.0.0.1:11434/api/generate
    │
    │  (3) Publish to:
    ├── starship.agent.<name>.status (for status bridge + all UIs)
...
Any component that reads `/tmp/starship-status.json` and renders it is a valid UI layer. Example for a terminal-based UI:

```python
import json, time
while True:
    data = json.loads(open('/tmp/starship-status.json').read())
    os.system('clear')
...
6. **No code changes needed** — `agent_daemon.py` is agent-agnostic. It reads the YAML and subscribes `starship.agent.<name>.command.>` automatically.
...
1. **Register in `agents/workflows.py`**:
   ...
   async def my_workflow(nc, payload):
       await nc.publish("starship.agent.proxy.command.do-thing", ...)
       await nc.publish("starship.agent.ergo.command.other-thing", ...)
       return {"status": "triggered", "workflow": "<name>"}
   ```
2. **Trigger** via:
   - NATS publish: `nats pub starship.workflow.<name> '{"workflow":"<name>"}'`
   - CLI: `starship workflow run <name>`
...
Then rebuild: `cd cli && go build -o starship`
...
3. **Start services**:
   ```bash
   sudo systemctl start starship-nats
   sudo systemctl start starship-staragent
   sudo systemctl start starship-agents
   sudo systemctl start starship-dashboard
   ```
4. **Verify**: `starship ping` and open `http://localhost:8899`
5. **Enable at boot**: `sudo systemctl enable starship-*`
...
| `conky/starship.conkyrc` | Lua | Conky: position, size, update interval, data sources, colors | conky |
...
| `STARSHIP_ROOT` | auto-detect | Various | Project root directory |
...
|---|---|---|---|---|
| AGENTS | `starship.agent.>`, `starship.workflow.>`, `starship.skill.>` | File | 72h | 1,000,000 |
| TELEMETRY | `starship.telemetry.>` | File | 24h | 500,000 |
...
2. staragent (Rust binary)
   └── Publishes starship.telemetry every 10s
...
4. starship-status.py (NATS → /tmp/starship-status.json bridge)

5. dashboard/server.py (port 8899, web UI + REST API)

6. message_history.py (JetStream consumer → /tmp/starship-history/)

7. UI Layer (all independent, all read /tmp/starship-status.json):
   ├── starship_tray.py (GTK3 system tray)
   ├── bridge_console.py (GTK4 native window)
   ├── conky (starship.conkyrc, desktop overlay)
   └── cinnamon desklet.js (Cinnamon desktop widget)

8. CLI (on-demand):
   └── starship (Go binary, connects NATS, runs command, exits)
```
...
```
multi-user.target
  └── starship-nats.service
        ├── starship-staragent.service  (BindsTo=nats)
        └── starship-agents.service     (BindsTo=nats)
              └── starship-dashboard.service  (After=agents)
```

### 8.3 Systemd Service Graph (Mesh/Advanced)

```
starship-agent-mesh.target
  ├── nats.service (Required)
  ├── staragent.service (Required)
  ├── starship-agent@proxy.service (Template)
  ├── starship-agent@romi.service (Template)
  ├── starship-agent@ergo.service (Template)
  └── starship-dashboard.target (BindsTo=mesh)
        ├── starship-status-bridge.service
        └── starship-dashboard-web.service
```
...
| starship_tray.py | tech | autostart .desktop | status.json |

### 8.5 State / Data File Locations

| File | Format | Written By | Read By | Purpose |
|---|---|---|---|---|
| `/tmp/starship-status.json` | JSON | starship-status.py | ALL UI + dashboard API | Current system state |
| `/tmp/starship-history/<date>.jsonl` | JSONL | message_history.py | dashboard /api/history | Historical messages |
...
| Command | Description |
|---|---|
| `starship ping` | Test NATS connectivity |
| `starship agent status` | Show all agent daemon statuses |
| `starship agent chat proxy` | Interactive chat with Proxy |
| `starship telemetry` | Show latest system telemetry |
| `starship workflow run security-audit` | Run security audit workflow |
| `starship skill list` | List installed skills |
| `make run-all` | Start all services (dev) |
| `nats pub starship.agent.ergo.command.status '{"command":"status"}'` | Send NATS command directly |
